...
tl.from(".about-image", { xPercent: 50, duration: 1.2 }).from(
  ".about-text",
  { xPercent: -50, opacity: 0, duration: 1.2 },
  "<"
);
...
const statObserver = new IntersectionObserver((entries, observer) => {
  ...
  entries.forEach((observerEntry) => {
    // 감시 중인 각 요소의 상태 정보를 하나씩 순회

    // 보일 때
    if (observerEntry.isIntersecting) {
      // observerEntry.isIntersecting이 true이면 → 화면에 나타난 상태
...
const skillObserver = new IntersectionObserver((entries, observer) => {
  //화면에 요소가 들어오거나 나갈 때 실행되는 감시자
  // entries: 화면에 들어오거나 나간 요소들의 정보가 담긴 배열
  // observer: 감시 중인 요소들을 제어할 수 있는 도구

  entries.forEach((entry) => {
    // 감시 중인 모든 요소들을 하나씩 순회하면서 처리
    if (entry.isIntersecting) {
      // 요소가 화면(Viewport)에 들어왔는지 확인 (true면 들어옴)

      setTimeout(() => {
        // 5초(5000ms) 뒤에 한번 실행되는 코드
